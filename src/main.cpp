...
uint256 hashGenesisBlock("0xebde1daf67d132d84c11a86679906edac74d3480732cfed497a0e8138d8b4956");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(537261531); // MelaniaCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MelaniaCoin Signed Message:\n";
...
{
    // MelaniaCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 18800000000;
	
			if(nHeight == 1)
            {
            nSubsidy = 299432700000000;
		}

    // Subsidy is cut in half every 788400 blocks, which will occur approximately every 1.5 years
    nSubsidy >>= (nHeight / 788400); // MelaniaCoin: 788400 blocks in ~1.5 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 360; // MelaniaCoin: 6 minutes/6 Blocks
static const int64 nTargetSpacing = 60; // MelaniaCoin: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MelaniaCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
				...
				if (pindexLast->nHeight+1 >= 9500) { DiffMode = 2; }
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MelaniaCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MelaniaCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3a1413814b3a932fa5b3469b675ed68812ae0ae2b4510efdd6a507587249877d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Melania Trump is now the First Lady of the White House 20th January 2017";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 18800000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("7af964583edfffff0ac8890993d5941bf66f558a8e2220a115a1e2b262fd4739e83bc1a020de72e37d8299476571df47136aecf8353c453feb4c5a44fe586aa500") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1485851936;
        block.nBits    = 537261531;
        block.nNonce   = 1887555760;

        if (fTestNet)
        {
            block.nTime    = 1485851936;
            block.nNonce   = 3838599204;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xc1aed6821cc7f9364d9dd18e79fa8bd960c943a832d607c8ed9036b18e73f68d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MelaniaCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MelaniaCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MelaniaCoinMiner : ProcessBlock, block not accepted");
    }
